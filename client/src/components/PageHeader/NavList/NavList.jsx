...
export default class NavList extends Component {
  ...
  render() {
    let navListStyle = this.props.expanded
      ? `${styles.navList} ${styles.navListShown}`
      : `${styles.navList}`;
    ...
    return (
      ...
      >
        <ul className={styles.navListContainer}>
          {this.state.navListEntries.map((entry, i) => (
            <li className={styles.navListEntry} key={i}>
              <Link to={`/${entry}`} className={styles.navListEntryAnchor}>
                {entry}
              </Link>
            </li>
          ))}
